...
pub trait CellAreaContextImpl: CellAreaContextImplExt + ObjectImpl {
    ...
    fn get_preferred_height_for_width(
        ...
        self.parent_get_preferred_height_for_width(cell_area_context, width)
    }
    ...
    fn get_preferred_width_for_height(
        ...
        self.parent_get_preferred_width_for_height(cell_area_context, height)
    }
...
impl<T: CellAreaContextImpl> CellAreaContextImplExt for T {
    fn parent_reset(&self, cell_area_context: &Self::Type) {
        unsafe {
            ...
            let parent_class =
                data.as_ref().get_parent_class() as *mut ffi::GtkCellAreaContextClass;
    ...
    fn parent_get_preferred_height_for_width(
        ...
        unsafe {
            ...
            if let Some(f) = (*parent_class).get_preferred_height_for_width {
                ...
                Some((minimum_size.assume_init(), natural_size.assume_init()))
            } else {
                None
            }
        }
...
unsafe extern "C" fn cell_area_context_get_preferred_height_for_width<T: CellAreaContextImpl>(
    ...
    let ret = imp.get_preferred_height_for_width(wrap.unsafe_cast_ref(), width);
    if let Some((min_height, nat_height)) = ret {
...
unsafe extern "C" fn cell_area_context_get_preferred_width_for_height<T: CellAreaContextImpl>(
    ...
    let ret = imp.get_preferred_width_for_height(wrap.unsafe_cast_ref(), height);
    if let Some((min_width, nat_width)) = ret {
