...
pub trait ToplevelExtManual {
    ...
    fn connect_compute_size<F: Fn(&Toplevel, &ToplevelSize) + 'static>(
        &self,
        f: F,
    ) -> SignalHandlerId;
}
...
impl<O: IsA<Toplevel>> ToplevelExtManual for O {
    fn connect_compute_size<F: Fn(&Toplevel, &ToplevelSize) + 'static>(
        ...
        unsafe extern "C" fn compute_size_trampoline<F: Fn(&Toplevel, &ToplevelSize) + 'static>(
            this: *mut ffi::GdkToplevel,
            size: *mut ffi::GdkToplevelSize,
            f: glib::ffi::gpointer,
        ) {
            let f: &F = &*(f as *const F);
            f(&from_glib_borrow(this), &from_glib_borrow(size))
        }
        unsafe {
